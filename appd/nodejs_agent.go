...
func (n NodeJSAgent) Contribute(layer libcnb.Layer) (libcnb.Layer, error) {
	...
	if !regexp.MustCompile(`require\(['"]appdynamics['"]\)`).Match(c) {
		n.Logger.Header("Requiring 'appdynamics' module")

		if err := ioutil.WriteFile(file, append([]byte("require('appdynamics');\n"), c...), 0644); err != nil {
